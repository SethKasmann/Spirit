...
Font::Font(std::string file, std::string text, size_t size)
    : SubTexture(file), _text(text), _surface(nullptr), _size(size) {
  _color.r = 255;
  _color.g = 255;
  _color.b = 255;
  _color.a = 255;
...
void Font::load() {
  ...
  if (_surface == nullptr) {
    std::cout << "Font::load: " << _file
              << " could not be converted to a font.\n"
...
void Font::generate(int w, int h) {
  ...
  float idnt_w = .5 / w;
  float idnt_h = .5 / h;
  _coordinates[0] = glm::vec3(_position.x / w + idnt_w,
                              (_position.y + _h) / h - idnt_h, _position.z);
  _coordinates[1] = glm::vec3((_position.x + _w) / w - idnt_w,
                              (_position.y + _h) / h - idnt_h, _position.z);
  _coordinates[2] = glm::vec3((_position.x + _w) / w - idnt_w,
