...
namespace spirit
{
    ...
    : _update(true), _scale(1.0f), _position(glm::vec2(x, y)), _projection(glm::mat4()), _modelview(glm::mat4())
    {}
    ...
    {
        _projection = glm::ortho(-w / 2.0f, static_cast<float>(w / 2), -h / 2.0f, static_cast<float>(h / 2));
    }
    ...
    const glm::mat4& Camera2d::get_projection() const
    {
        return _projection;
    }
    ...
    const glm::mat4& Camera2d::get_modelview()
    {
        if(_update)
        {
            _modelview = glm::translate(glm::mat4(), glm::vec3(_position.x, _position.y, 0.0f));
