...
class Text {
public:
  Text(const glm::vec3 &pos, const Texture &texture, const std::string &key,
       const std::string &text, uint8_t r, uint8_t g, uint8_t b,
       uint8_t a = 255)
      : _position(pos), _w(0), _h(0) {
    init(texture, key, text, r, g, b, a);
  }
  void init(const Texture &texture, const std::string &key,
            const std::string &text, uint8_t r, uint8_t g, uint8_t b,
            uint8_t a = 255) {
    ...
    for (auto &c : text) {
      ...
      _characters.push_back(
          Object2d(temp, texture, std::string(key + _tag + c), r, g, b, a));
  ...
  void set_text(const Texture &texture, const std::string &key,
                const std::string &text, uint8_t r, uint8_t g, uint8_t b,
                uint8_t a = 255) {
    _w = 0;
    _h = 0;
  ...
  const Object2d *get_ptr() const { return _characters.data(); }
  size_t get_size() const { return _characters.size(); }
